...
import { computeJiraDistrict, JIRA_DISTRICT } from '../../utils/openWorldJiraDistrict';

// OpenWorld's JIRA sprint district (roadmap 3.7): the current sprint's tickets become a small
...
// component only renders + animates. Mirrors OpenWorldTaskQueue / OpenWorldGoalMonuments.
...
export default function OpenWorldJiraDistrict({ jiraTickets, settings }) {
  ...
  return (
    <group>
      ...
      {overflow > 0 && overflowPosition && (
        <OpenWorldLabel position={[overflowPosition[0], 1.4, overflowPosition[2]]} fontSize={0.5} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9}>
          {`+${overflow} MORE`}
        </OpenWorldLabel>
      )}

      {/* District title + sprint progress */}
      <OpenWorldLabel position={[base[0], 12, base[2] - 2]} fontSize={1.2} color="#3b82f6" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={30}>
        SPRINT
      </OpenWorldLabel>
      <OpenWorldLabel position={[base[0], 11.1, base[2] - 2]} fontSize={0.72} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={30}>
