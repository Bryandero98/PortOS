...
// OpenWorld's voice-agent district marker (roadmap 2.4): a modest ground-level beacon
...
export default function OpenWorldVoiceMarker({ voiceState, settings }) {
  ...
  return (
    <group position={position}>
      ...
      <OpenWorldLabel position={[0, poleHeight + beaconRadius * 0.6 + 1.6, 0]} fontSize={1} color={color} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={16}>
        VOICE
      </OpenWorldLabel>
      <OpenWorldLabel position={[0, poleHeight + beaconRadius * 0.6 + 0.9, 0]} fontSize={0.7} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={16}>
        {label}
      </OpenWorldLabel>
    </group>
  );
