...
export async function writeRaw(id, record) {
  ...
  await query(
    `INSERT INTO fableloom_stories (id, name, universe_id, series_id, data, created_at, updated_at)
    ...
    [id, record.name, record.universeId ?? null, record.seriesId ?? null, JSON.stringify(record), createdAt, updatedAt],
  );
  return record;
}
