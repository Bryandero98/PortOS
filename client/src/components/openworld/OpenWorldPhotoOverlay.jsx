...
import { PHOTO_PRESETS, getPreset, cyclePreset, buildPostcardStats, screenshotFilename } from '../../utils/openWorldPhotoMode';
...
// non-visual logic (presets, caption lines, filename) lives in the pure openWorldPhotoMode helper.

// Composite the raw screenshot data URL onto a postcard: the city image with a subtle vignette,
// an OPENWORLD title, and the stats caption along the bottom. Returns a PNG data URL. Runs on a
...
function composePostcard(dataUrl, statLines) {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      ctx.fillText('OPENWORLD', pad, img.height - pad - Math.round(img.height * 0.04));
...
export default function OpenWorldPhotoOverlay({ active, presetId, onPresetChange, onExit, captureFnRef, statsSnapshot, dofEnabled = true, onToggleDof }) {
