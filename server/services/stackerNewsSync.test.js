...
const query = vi.fn(async (sql) => {
  if (sql.startsWith('SELECT * FROM stacker_news_accounts')) return { rows: [{ id: accountId, label: 'Example', username: 'example_user', enabled: true, monitoring_enabled: true, monitoring_interval_minutes: 30, analysis_enabled: false, text_model: '', vision_model: '', rules: {}, policy_version: 'v1' }] };
  if (sql.startsWith('SELECT api_key_enc')) return { rows: [{ api_key_enc: 'ciphertext' }] };
  if (sql.startsWith('SELECT * FROM stacker_news_territories')) return { rows: [{ id: territoryId, account_id: accountId, slug: 'art', label: 'Art', is_owned: true, monitoring_enabled: null, inherit_account_rules: true, rules: {}, remote_settings: {} }] };
