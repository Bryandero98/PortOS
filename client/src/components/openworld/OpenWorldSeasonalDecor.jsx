...
import { computeSeasonalDecor, SEASONAL_DECOR } from '../../utils/openWorldSeasonalDecor';

// OpenWorld's seasonal decorations (roadmap 3.5 follow-up, #824): date-driven city dressing
...
// phase offset) gated on the quality dial (mirrors OpenWorldArtifacts / OpenWorldGoalMonuments).
...
export default function OpenWorldSeasonalDecor({ date, settings }) {
  // Re-resolve the theme as real time advances so the dressing turns over at midnight / a holiday
  // boundary while OpenWorld stays open — an hourly tick is plenty for a calendar-day theme and is
  ...
  return (
    <group>
      ...
      <OpenWorldLabel position={[base[0], 34, base[2]]} fontSize={1.6} color={color} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={40}>
        {label}
      </OpenWorldLabel>
    </group>
  );
