...
- `resolve:round` / `resolve:no-change` / `resolve:isolate` — one resolver ATTEMPT and what it wrote. `arcFieldsEdited` / `volumesEdited` / `characterArcsEdited` / `episodesEdited` count the records it rewrote, so read all four: `episodesEdited: 0` is EXPECTED at the `arcSpine` scope, whose resolver may only patch the arc and volumes, and calling that round a no-op on the episode count alone is a misreading. (The `beatContinuity` scope is the exception — its resolver only rewrites episode beats, so `episodesEdited` there is the whole account.)
- `noChangeReason` — on an attempt that wrote nothing, WHICH kind of nothing, and they call for different fixes: `exact-edits-rejected` (its anchors no longer matched the text), `edits-out-of-scope` (it answered at an altitude the gate cannot apply), `edits-named-no-finding` (its edits named none of the findings it was handed), `edits-matched-existing` (it re-authored text the plan already held), `isolated-candidate-rejected` (a per-finding attempt too broad to be one), `no-findings` (there was nothing to resolve), `no-edits-returned` (it proposed nothing at all — a content-level refusal more rounds will not fix).
