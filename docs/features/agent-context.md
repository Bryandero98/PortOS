...
PortOS can expose a bounded, read-only context surface to agents running on the same machine. The surface uses MCP Streamable HTTP, is disabled by default, and never initiates an LLM request.

## Enable it

Open **Settings → API Access → Agent Context (MCP)** and enable **local MCP context**. The setting is stored machine-locally in the existing `settings.json` store; no context data is copied into a new store.
...
All advertised tools carry MCP `readOnlyHint: true`, `destructiveHint: false`, `idempotentHint: true`, and `openWorldHint: false`. Their advertised JSON Schemas are generated from the same Zod schemas that validate runtime calls and outputs.
