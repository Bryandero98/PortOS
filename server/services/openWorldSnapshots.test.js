...
describe('openWorldSnapshots service', () => {
  let dataDir;
  let snapshotsFile;

  beforeEach(() => {
    vi.resetModules();
    dataDir = mkdtempSync(join(tmpdir(), 'portos-openworld-snap-'));
  ...
  async function loadService({ settings = {}, sources = {}, reject = [], fileOverrides = {} } = {}) {
    ...
    return import('./openWorldSnapshots.js');
  }
  ...
  it('resolves config defaults when settings absent, and overrides when present', async () => {
    ...
    const svcCustom = await loadService({
      settings: { openWorldSnapshots: { enabled: false, intervalMinutes: 15, maxSnapshots: 50 } },
    });
  ...
  it('falls back to defaults field-by-field for invalid settings values', async () => {
    const svc = await loadService({
      settings: { openWorldSnapshots: { enabled: 'yes', intervalMinutes: 0, maxSnapshots: 5 } },
    });
  ...
  it('enforces the maxSnapshots cap by dropping the oldest frames', async () => {
    const svc = await loadService({ settings: { openWorldSnapshots: { maxSnapshots: 10 } } });
