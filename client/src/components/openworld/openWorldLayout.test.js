...
// spacing = BUILDING_PARAMS.spacing = 12, DISTRICT_PARAMS.gap = 4 (see openWorldConstants.js)
const online = (id) => ({ id, overallStatus: 'online', archived: false });

describe('computeOpenWorldLayout', () => {
  it('lays active apps around a reserved AI Core plaza', () => {
    const pos = computeOpenWorldLayout([online('a'), online('b'), online('c'), online('d')]);
  ...
  it('pushes a lone centered app to the front edge of the core plaza', () => {
    const pos = computeOpenWorldLayout([online('solo')]);
  ...
  it('offsets archived apps into a warehouse grid clear of the core plaza on sparse installs', () => {
    const pos = computeOpenWorldLayout([
      online('a'),
  ...
  it('never stacks two downtown buildings when clearing the core plaza', () => {
    ...
    const pos = computeOpenWorldLayout(Array.from({ length: 9 }, (_, i) => online(String(i))));
    const seen = new Set();
