...
describe('OpenWorldFocusPanel', () => {
  ...
  it('lists active/failed agents assigned to the app', () => {
    ...
    render(<OpenWorldFocusPanel app={app} agents={agents} />);
  ...
  it('fires onClose from the close and back actions', () => {
    const onClose = vi.fn();
    render(<OpenWorldFocusPanel app={app} agents={[]} onClose={onClose} />);
  ...
  it('renders the not-found fallback with a return-to-overview action', () => {
    const onClose = vi.fn();
    render(<OpenWorldFocusPanel app={null} notFound onClose={onClose} />);
