...
//
// Mounted only at night (OpenWorldScene gates on !showGradientBackground), so the 2.8MB
// panorama isn't fetched/decoded or PMREM-processed in daylight.
export default function OpenWorldGalaxySky({ settings }) {
  const { gl, scene } = useThree();
  const texture = useLoader(THREE.TextureLoader, GALAXY_TEXTURE_URL);
  const nightOpacity = Math.max(0, Math.min(1, 1 - openWorldDayMix(settings)));
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // OpenWorldScene once this unmounts) isn't left dimmed by the last night value.
