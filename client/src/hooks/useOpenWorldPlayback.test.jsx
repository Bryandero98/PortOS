...
const getOpenWorldSnapshots = vi.fn();
vi.mock('../services/apiOpenWorld.js', () => ({
  getOpenWorldSnapshots: (...a) => getOpenWorldSnapshots(...a),
}));

import { useOpenWorldPlayback, PLAYBACK_SPEEDS } from './useOpenWorldPlayback.js';
...
describe('useOpenWorldPlayback', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getOpenWorldSnapshots.mockResolvedValue({ total: 3, snapshots: mkFrames(3) });
  });

  it('loads the series on enter and starts at the most recent frame', async () => {
    const { result } = renderHook(() => useOpenWorldPlayback());
  ...
  it('flags an error (distinct from empty) when the fetch fails', async () => {
    getOpenWorldSnapshots.mockRejectedValue(new Error('network down'));
    const { result } = renderHook(() => useOpenWorldPlayback());
    await act(async () => { await result.current.enter(); });
  ...
  it('a real empty history is not an error', async () => {
    getOpenWorldSnapshots.mockResolvedValue({ total: 0, snapshots: [] });
    const { result } = renderHook(() => useOpenWorldPlayback());
    await act(async () => { await result.current.enter(); });
  ...
  it('filters out frames with an unsupported schemaVersion', async () => {
    getOpenWorldSnapshots.mockResolvedValue({
