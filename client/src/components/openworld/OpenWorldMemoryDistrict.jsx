...
import { computeMemoryDistrict, MEMORY_DISTRICT } from '../../utils/openWorldMemoryDistrict';

// OpenWorld's memory / knowledge district (roadmap 3.2): the user's long-term memory graph
...
// be classified, calm when the inbox is clear. Mirrors OpenWorldGoalMonuments / OpenWorldBackupVault:
...
function CrystalCluster({ cluster, glowRef, isGlow, dayMix = 0 }) {
  ...
  return (
    <group position={position}>
      ...
      <OpenWorldLabel position={[0, height + 1.3, 0]} fontSize={0.6} color={color} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9}>
        {label}
      </OpenWorldLabel>
      <OpenWorldLabel position={[0, height + 0.7, 0]} fontSize={0.42} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9}>
        {count}
      </OpenWorldLabel>
    </group>
  );
...
export default function OpenWorldMemoryDistrict({ memoryGraph, inboxDepth = 0, settings }) {
  ...
  return (
    <group>
      ...
      <group position={base}>
        ...
        {inboxActive && (
          <OpenWorldLabel position={[0, 1.2, 0]} fontSize={0.5} color="#06b6d4" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9}>
            {`${inboxDepth} TO SORT`}
          </OpenWorldLabel>
        )}
      </group>
      ...
      {/* District title */}
      <OpenWorldLabel position={[base[0], 14, base[2] - 3]} fontSize={1.3} color="#8b5cf6" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={30}>
        MEMORY
      </OpenWorldLabel>
      <OpenWorldLabel position={[base[0], 13, base[2] - 3]} fontSize={0.75} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={30}>
