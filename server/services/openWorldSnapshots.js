/**
 * OpenWorld Snapshot Store
 *
 * Periodically captures a compact snapshot of the OpenWorld's derived state
...
// `openWorldSnapshots` settings key behave sanely without a migration.
...
export async function getSnapshotConfig() {
  ...
  const c = settings?.openWorldSnapshots || settings?.[legacySnapshotSettingsKey] || {};
  return {
