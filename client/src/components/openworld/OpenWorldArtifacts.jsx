...
function Artifact({ artifact, dayMix = 0 }) {
  ...
  return (
    <group position={position}>
      ...
      <OpenWorldLabel position={[0, ph + emblemSize * 1.7, 0]} fontSize={0.5} color={color} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={6}>
        {label}
      </OpenWorldLabel>
    </group>
  );
...
export default function OpenWorldArtifacts({ character, goals, productivityData, settings }) {
  ...
  return (
    <group>
      ...
      <OpenWorldLabel position={[base[0], 10, base[2] + 4]} fontSize={1.2} color="#f59e0b" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={24}>
        ACHIEVEMENTS
      </OpenWorldLabel>
      <OpenWorldLabel position={[base[0], 9, base[2] + 4]} fontSize={0.7} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={24}>
        {`${total} EARNED`}
      </OpenWorldLabel>
    </group>
  );
