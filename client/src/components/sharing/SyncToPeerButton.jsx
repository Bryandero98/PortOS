...
export default function SyncToPeerButton({
  ...
  // in-card `absolute z-30` popover is painted UNDER the next card).
  // `usePopoverPosition` owns the fixed placement: right-aligned to the trigger,
  // clamped into the viewport, flipped above when there is no room below, and
  // re-flowed on capture-phase scroll. The peer list changes the menu's height
  // as it loads, so it re-measures via contentDeps.
  const { triggerRef, popoverRef, style: menuStyle } = usePopoverPosition({
    ...
    gap: 4,
    position: 'below',
    contentDeps: [loading, peers.length, subscriptions.length]
  });
  ...
  const handleToggle = async (peer) => {
    ...
    if (existing) {
      ...
      // apiCore helper must not fire its own (per AGENTS.md "Custom catch ⇒
