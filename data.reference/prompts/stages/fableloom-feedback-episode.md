...
- You may revise existing scenes' `title`, complete `prose`, `imagePrompt`, `videoPrompt`, `cameraMovement`, `isEnding`, and `endingLabel`.
...
- Each scene is one continuous camera cut and one separately rendered video clip. A revised `videoPrompt` must contain visible action, one primary move, pace, atmosphere, and final beat—never an edit or second setup. Choose `cameraMovement` from this vocabulary:
