...
This is an unattended run, but it is not browserless when the target has a web UI. PortOS provides a managed Chromium browser for CoS agents over Chrome DevTools Protocol (CDP). The agent-facing browser bridge is separate from that managed browser: the empty-registry case (agent.browsers.list() === []), getForUrl() returning "No browser is available", or an otherwise unusable agent-browser binding is a provider-bridge failure, not evidence that PortOS's managed Chromium or CDP endpoint is unavailable. Do not skip live UI verification because the provider bridge is empty, unusable, or no human is present.

- When the agent browser bridge is empty or unusable, query the configured PortOS health endpoint (usually /api/browser/health) and the CDP /json/version or /json/list endpoints before stopping. A healthy PortOS health/CDP response overrides the provider's "No browser is available" response.
