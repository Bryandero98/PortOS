...
describe('desiredEndState', () => {
  ...
  it('drives NEEDS_PR past the PR to a merge when autoMerge is on', () => {
    ...
    expect(instruction).toContain('/do:pr --merge');
    expect(instruction).toContain('gh pr checks <num>');
  ...
  it('makes both merge-bound states wait for CI in-session rather than queueing --auto', () => {
    for (const state of ['NEEDS_PR', 'IN_REVIEW']) {
      const instruction = desiredEndState(state, {});
      expect(instruction).toContain('--required --watch --fail-fast');
      expect(instruction).not.toContain('--auto');
    }
  });

  it('names the real PR number when the branch already has one', () => {
    const instruction = desiredEndState('IN_REVIEW', {}, { prNumber: 42 });
    expect(instruction).toContain('gh pr checks 42 --required --watch --fail-fast');
