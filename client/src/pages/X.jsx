...
export default function XPage() {
  ...
  // The diagnostic and "open on X" actions resolve `account.username` SERVER-side
  // from the saved record, so an unsaved handle edit would silently run against the
  // old handle while the form shows the new one. Dirtiness is deliberately NOT
  // scoped to `accountFormMode === 'edit'`: closing the drawer keeps the draft, so
  // the mismatch outlives the drawer. Mirrors StackerNews.jsx's gate.
  const accountFormDirty = Boolean(selected) && !sameAccount(accountForm, savedAccountForm);
  const accountActionsDisabled = accountFormDirty || Boolean(busy);
  const discardAccountEdits = () => { setAccountForm(savedAccountForm); setAccountFormError(''); };
  const unsavedAccountNotice = accountFormDirty && <div className="mt-4 flex flex-wrap items-center gap-2 rounded border border-port-warning/40 bg-port-warning/10 p-3"><p className="text-sm text-port-warning">Unsaved changes to this account. Save it before running these checks.</p><button type="button" className={secondaryButton} disabled={Boolean(busy)} onClick={discardAccountEdits}>Discard changes</button></div>;
