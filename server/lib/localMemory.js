...
/**
 * The same refusal for the SGLang container. The stop command deliberately has
 * no `--profile`: the vLLM stack is upstream's compose project, whose services
 * sit behind a `single` profile, while the SGLang compose file is generated by
 * `lib/sglangQwenRecipe.js` and declares one unprofiled service. Copying vLLM's
 * `--profile single` here would print a command that stops nothing.
 */
