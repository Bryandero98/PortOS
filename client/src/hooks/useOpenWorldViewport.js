...
// Coordinated responsive breakpoints for the OpenWorld HUD. The HUD is a fixed
...
export function classifyOpenWorldViewport(width, height = Infinity) {
...
export default function useOpenWorldViewport() {
  const [mode, setMode] = useState(() =>
    typeof window === 'undefined' ? 'desktop' : classifyOpenWorldViewport(window.innerWidth, window.innerHeight),
  );

  useEffect(() => {
    const onResize = () => setMode(classifyOpenWorldViewport(window.innerWidth, window.innerHeight));
