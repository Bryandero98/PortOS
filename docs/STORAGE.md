...
- `creative_commissions` — Creative Commissions (Autonomous Creation Engine, #2657/#2686): standing recurring creative briefs that fire on a cron cadence and drive the Creative Director directive pipeline unattended. One row per commission, the full sanitized record in `data` JSONB with `name`/`enabled`/`created_at`/`updated_at` mirrored into columns for the scheduler's "arm every enabled commission" query. The **brief** (including bounded `brief.musicTaste` configuration) and feedback federate; the machine-local `schedule`, `runs[]`, `assignment`, and `enabled` fields never cross peers, so only the owning machine fires a cron and keeps local taste-recipe, selected renderer, and completed-audio provenance. Brief updates use a `briefUpdatedAt` LWW clock, with `deleted`/`deleted_at` tombstones for brief federation. Raw Digital Twin/Spotify records and full responses remain machine-local. Adapter: `server/services/creativeCommissions/db.js`, selected pg-vs-file by `server/services/creativeCommissions/store.js` (file backend is the `NODE_ENV=test`/`MEMORY_BACKEND=file` escape hatch only).
