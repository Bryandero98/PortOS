...
import OpenWorldTubeLine from './OpenWorldTubeLine';
...
import { computeGoalMonuments, computeGoalForest, MONUMENTS, FOREST } from '../../utils/openWorldGoalMonuments';

// OpenWorld's goal monuments (roadmap 2.7): each life goal is a structure in a
...
// OpenWorldBackupVault / OpenWorldHealthTower.
...
function Monument({ monument, shimmerRef, isShimmer, dayMix = 0 }) {
  ...
  return (
    <group position={position}>
      ...
      <OpenWorldLabel position={[0, height + 1.4, 0]} fontSize={isSpire ? 0.85 : 0.7} color={color} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9} fillOpacity={opacity}>
        {monument.title}
      </OpenWorldLabel>
      <OpenWorldLabel position={[0, height + 0.7, 0]} fontSize={0.55} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9} fillOpacity={opacity}>
...
function GoalForest({ forest, shimmerRef, shimmerId, dayMix = 0 }) {
  return (
    <group>
      {forest.clusters.map((cluster) => (
        <group key={cluster.spire.id}>
          <Monument monument={cluster.spire} shimmerRef={shimmerRef} isShimmer={cluster.spire.id === shimmerId} dayMix={dayMix} />
          {cluster.children.map((child) => (
            <group key={child.id}>
              <Monument monument={child} shimmerRef={shimmerRef} isShimmer={child.id === shimmerId} dayMix={dayMix} />
              {child.descendantCount > 0 && (
                <OpenWorldLabel position={[child.position[0], child.height + 1.9, child.position[2]]} fontSize={0.42} color="#64748b" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9}>
          ...
          {cluster.childOverflow > 0 && (
            <OpenWorldLabel position={[cluster.spire.position[0], cluster.spire.height + 2.1, cluster.spire.position[2]]} fontSize={0.5} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9}>
...
export default function OpenWorldGoalMonuments({ goals, settings }) {
  const { tintStructure } = useOpenWorldPalette();
  ...
  return (
    <group>
      {useForest ? (
        <GoalForest forest={forest} shimmerRef={shimmerRef} shimmerId={shimmer?.id} dayMix={dayMix} />
      ) : (
        <>
          ...
          {overflow && (
            <group position={overflow.position}>
              ...
              <OpenWorldLabel position={[0, MONUMENTS.minHeight + 1, 0]} fontSize={0.6} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9}>
                {`+${overflow.count} MORE`}
              </OpenWorldLabel>
            </group>
          )}
      ...
      <OpenWorldLabel position={[base[0], 16, base[2] - 3]} fontSize={1.4} color="#22c55e" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={30}>
        GOALS
      </OpenWorldLabel>
      <OpenWorldLabel position={[base[0], 15, base[2] - 3]} fontSize={0.8} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={30}>
