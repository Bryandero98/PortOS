...
import {
  ...
  mergeFrameIntoOpenWorldProps,
  buildPlaybackStats,
  SUPPORTED_SNAPSHOT_SCHEMA_VERSION,
} from './openWorldPlaybackFrame.js';
...
describe('mergeFrameIntoOpenWorldProps', () => {
  it('returns null for an unplayable frame so the page keeps live data', () => {
    expect(mergeFrameIntoOpenWorldProps(frame({ schemaVersion: 99 }), { apps: liveApps })).toBeNull();
  });

  it('maps the faithfully-driveable scene props (apps/agentMap/cos/backup/character)', () => {
    const props = mergeFrameIntoOpenWorldProps(frame(), { apps: liveApps });
  ...
  it('passes null (not a fabricated 0/empty) when cos/backup/character were unavailable at capture', () => {
    const props = mergeFrameIntoOpenWorldProps(
