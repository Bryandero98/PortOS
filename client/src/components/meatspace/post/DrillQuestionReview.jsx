...
const isUnanswered = (q) => {
  if (Array.isArray(q.answered)) return !q.answered.some(answer => answer?.value != null && String(answer.value).trim() !== '');
...
// memory-fill-blank carries one indexed response per generated blank. Use the
// response-local answer key in history so partial credit is visible even after
// the original generated question is no longer present.
function expectedForDisplay(q) {
  if (Array.isArray(q.answered) && q.answered.length) {
    return q.answered.map(answer => answer?.expected || '—').join(' / ');
  }
...
function answeredForDisplay(type, q) {
  if (type === 'memory-fill-blank' && Array.isArray(q.answered)) {
    return q.answered.map(answer => answer?.value || '—').join(' / ');
  }
              ...
              <tr key={i} className={`border-b border-port-border/30 ${rowTint(q.correct, unanswered)}`}>
                ...
                <td className="py-1 pr-2 font-mono">
                  {unanswered
                    ...
                    : answeredForDisplay(type, q)}
                </td>
                <td className="py-1 pr-2 font-mono text-port-success">{formatGenericValue(type, expectedForDisplay(q))}</td>
                <td className="py-1 pr-2 text-center">
                  <div className="inline-flex items-center gap-1">
                    <ResultIcon correct={q.correct} unanswered={unanswered} />
                    {type === 'memory-fill-blank' && Array.isArray(q.answered) && (
                      <span className="text-[11px] text-gray-400">
                        {q.answered.filter(answer => answer?.correct).length}/{q.answered.length}
                      </span>
                    )}
                  </div>
                </td>
