...
import { getOpenWorldSnapshots } from '../services/apiOpenWorld.js';
import { isPlayableFrame, buildPlaybackStats } from '../lib/openWorldPlaybackFrame.js';

// Transport state for the OpenWorld timeline scrubber (issue #967). Owns the
// snapshot series, the current frame index, and play/pause/speed so OpenWorld.jsx
// stays lean. Pure-UI: it reads the snapshot API and steps an index; the page
// turns the current frame into scene props via lib/openWorldPlaybackFrame.js.

export const PLAYBACK_SPEEDS = [1, 2, 4]; // × frames/sec
const BASE_INTERVAL_MS = 1000; // 1×: advance one frame per second

export function useOpenWorldPlayback() {
  ...
  const enter = useCallback(async () => {
    ...
    const res = await getOpenWorldSnapshots({ silent: true }).catch(() => null);
