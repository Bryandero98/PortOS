...
vi.mock('./OpenWorldPaletteContext', () => ({ useOpenWorldPalette: () => ({ ground: '#22d3ee' }) }));

import OpenWorldLights from './OpenWorldLights';

const pointLightCount = (settings) => {
  const { container } = render(<OpenWorldLights settings={settings} />);
  return container.getElementsByTagName('pointLight').length;
};

describe('OpenWorldLights quality-tier culling', () => {
  ...
  it('keeps every light when no settings are supplied', () => {
    // Legacy/absent payloads fall through openWorldShowDetail's particleDensity default of 1,
  ...
  it('keeps the key, fill and glow lights on the low tier', () => {
    ...
    const { container } = render(<OpenWorldLights settings={{ effectiveTier: 'low' }} />);
