...
import { computeFederationHorizon, FEDERATION } from '../../utils/openWorldFederation';
...
function Monolith({ position, width, height, color, opacity, label, sublabel, online, animate, dayMix = 0 }) {
  ...
  return (
    <group position={position}>
      ...
      {label && (
        <OpenWorldLabel position={[0, height + 2.2, 0]} fontSize={1.6} color={color} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={20}>
          {label}
        </OpenWorldLabel>
      )}
      {sublabel && (
        <OpenWorldLabel position={[0, height + 0.9, 0]} fontSize={1.0} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={20}>
          {sublabel}
        </OpenWorldLabel>
      )}
    </group>
...
export default function OpenWorldFederationHorizon({ instances, settings }) {
