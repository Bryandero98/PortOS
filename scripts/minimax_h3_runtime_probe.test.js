...
// The probe reaches `_runner_common` and `_minimax_h3_mlx_pins` by bare name,
// which works when Python runs it as a script because the script's own
// directory lands on sys.path. Loading it through spec_from_file_location does
// not do that, so the harness has to.
const importProbe = [
  ...
  'pins = sys.modules["_minimax_h3_mlx_pins"]',
...
const fakeCheckout = ({ drop = null, plainProcessor = false } = {}) => {
  ...
  return [
    ...
    + '    "__MERGE__", pins.PINNED_BROADCAST_MERGE))',
  ].join('\n');
};
...
describe.skipIf(!pyBin)('minimax_h3_runtime_probe.py', () => {
  ...
  it('fails Install / Repair when upstream stopped merging by broadcast', () => {
    const fixedUpstream = [
      ...
      '    source.replace(pins.PINNED_BROADCAST_MERGE, "a real scatter"))',
    ].join('\n');
    const output = runPython([
  ...
  it.each([
    // Dropping the whole module and dropping just the leaf attribute both walk
    // the same dotted path in require_borrowed_attr, so both report the full
    // attribute it was trying to resolve rather than the shorter module name.
    ['mlx_vlm.utils', /no longer exposes mlx_vlm\.utils\.sanitize_weights/],
    ['mlx_vlm.utils.sanitize_weights', /no longer exposes mlx_vlm\.utils\.sanitize_weights/],
    ['mlx_vlm.models.qwen3_vl.qwen3_vl', /no longer exposes mlx_vlm\.models\.qwen3_vl\.qwen3_vl\.Model\.merge_input_ids_with_image_features/],
    [
...
describe('MiniMax H3 pin facts', () => {
  ...
  // BORROWED_MLX_VLM_ATTRS is a hand-kept mirror of what the corrections import.
  ...
  it('probes every mlx-vlm symbol the corrections import', () => {
    const covered = new Set(
      [...readScript('_minimax_h3_mlx_pins.py').matchAll(/\("(mlx_vlm[\w.]*)",\s*"([\w.]+)"/g)]
