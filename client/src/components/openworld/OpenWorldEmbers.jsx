...
import { seededRand } from './openWorldConstants';
...
export default function OpenWorldEmbers({ settings }) {
  const pointsRef = useRef();
  const matRef = useRef();
  // Follow OpenWorldParticles' quality-dial pattern: scale ember count down under the
  ...
  const { positions, sizes, speeds, phases, colors, count } = useMemo(() => {
    ...
    // Seeded, like OpenWorldDataRain: `density` is clamped for the first 1.2s of
