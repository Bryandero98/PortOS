...
vi.mock('../components/openworld/audio/openWorldAudioEngine', () => ({
  initAudio: vi.fn(() => ({})),
  setMusicVolume: vi.fn(),
  setSfxVolume: vi.fn(),
  scheduleCleanup: vi.fn(),
}));
vi.mock('../components/openworld/audio/openWorldSynthMusic', () => ({
  startMusic: vi.fn(),
  stopMusic: vi.fn(() => 0),
  setSoundscape: vi.fn(),
}));
vi.mock('../components/openworld/audio/openWorldSoundEffects', () => ({
  playSfx: vi.fn(),
}));

import useOpenWorldAudio from './useOpenWorldAudio.js';
import { setSoundscape } from '../components/openworld/audio/openWorldSynthMusic';
...
describe('useOpenWorldAudio soundscape override', () => {
  it('applies the live soundscape when no override is set', () => {
    renderHook(() => useOpenWorldAudio(baseSettings, LIVE));
    primeAudio();
  ...
  it('applies the forced chord set while the override is set', () => {
    const { rerender } = renderHook(
      ({ settings }) => useOpenWorldAudio(settings, LIVE),
  ...
  it('keeps the forced mood when live system state changes underneath it', () => {
    const { rerender } = renderHook(
      ({ settings, soundscape }) => useOpenWorldAudio(settings, soundscape),
  ...
  it('ignores an unknown stored override value and stays on live state', () => {
    renderHook(() => useOpenWorldAudio({ ...baseSettings, soundscapeOverride: 'stale-mood' }, LIVE));
    primeAudio();
    expect(lastSoundscape()).toMatchObject({ mood: LIVE.mood, chordSet: LIVE.chordSet });
  });
  ...
  it('does not re-apply when a poll recomputes an identical soundscape', () => {
    const { rerender } = renderHook(
      ({ soundscape }) => useOpenWorldAudio(baseSettings, soundscape),
