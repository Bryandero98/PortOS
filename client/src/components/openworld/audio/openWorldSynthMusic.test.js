...
import { CHORD_SETS } from '../../../utils/openWorldSoundscape';
import { SYNTH_TIMING } from '../../../lib/lookaheadTransport';

// Minimal fake Web Audio graph for openWorldSynthMusic.js. The module pulls its
// context/output gain from openWorldAudioEngine's getAudioContext()/getMusicGain()
...
// Constants read straight from openWorldSynthMusic.js (not exported, so pinned
...
beforeEach(async () => {
  ...
  synth = await import('./openWorldSynthMusic.js');
});

afterEach(() => {
  // openWorldSynthMusic schedules real setInterval timers on startMusic(); stop
