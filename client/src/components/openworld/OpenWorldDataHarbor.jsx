...
import { computeDataHarbor, DATA_HARBOR } from '../../utils/openWorldDataHarbor';
import { WORLD, AVENUE_WIDTH } from '../../utils/openWorldPlan';

// OpenWorld's Data Harbor: a pier district over the bay (master plan, north shore) that
// renders GET /api/openworld/introspection — the database quay (one disk-stack silo per Postgres
...
// rack opens a holographic detail card. Mirrors OpenWorldMemoryDistrict: the pure helper does
...
function TableSilo({ silo, color, onSelect, registerRing, dayMix }) {
  ...
  return (
    <group position={[silo.x, DATA_HARBOR.deckY, silo.z]}>
      ...
      <OpenWorldLabel position={[0, silo.height + 1.5, 0]} fontSize={0.42} color={color} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={7}>
        {silo.label}
      </OpenWorldLabel>
      <OpenWorldLabel position={[0, silo.height + 1.05, 0]} fontSize={0.3} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={7}>
        {silo.sublabel}
      </OpenWorldLabel>
    </group>
  );
...
function DomainRack({ rack, accent, tintStructure, onSelect, dayMix }) {
  ...
  return (
    <group position={[rack.x, DATA_HARBOR.deckY, rack.z]}>
      ...
      <OpenWorldLabel position={[0, rack.height + 1.2, 0]} fontSize={0.42} color={accent} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={7}>
        {rack.label}
      </OpenWorldLabel>
      <OpenWorldLabel position={[0, rack.height + 0.75, 0]} fontSize={0.3} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={7}>
        {rack.sublabel}
      </OpenWorldLabel>
    </group>
  );
...
export default function OpenWorldDataHarbor({ introspection, settings }) {
  const { accent, tintStructure, getAccentColor } = useOpenWorldPalette();
    ...
    <group>
      ...
      {dbDown && (
        <group position={[decks[0].x, DATA_HARBOR.deckY, decks[0].z]}>
          ...
          <OpenWorldLabel position={[0, 3.4, 0]} fontSize={0.7} color={OFFLINE_COLOR} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={10}>
            DB OFFLINE
          </OpenWorldLabel>
        </group>
      )}
      ...
      {obelisk && (
        <group position={[obelisk.x, DATA_HARBOR.deckY, obelisk.z]}>
          ...
          <OpenWorldLabel position={[0, 5, 0]} fontSize={0.42} color={accent} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={8}>
      ...
      <OpenWorldLabel position={[bx, 11, bz - 2]} fontSize={1.4} color={accent} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={30}>
        DATA HARBOR
      </OpenWorldLabel>
      <OpenWorldLabel position={[bx, 9.9, bz - 2]} fontSize={0.7} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={34}>
        {dbDown
      ...
      {(overflow.tables > 0 || overflow.domains > 0) && (
        <OpenWorldLabel position={[bx, 9.1, bz - 2]} fontSize={0.45} color="#64748b" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={30}>
