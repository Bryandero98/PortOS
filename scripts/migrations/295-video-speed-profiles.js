/**
 * Attach `speedProfiles` to shipped video models (issue #4875) in existing
 * installs.
 *
 * `data.reference/media-models.json` ships the profiles for fresh installs and
 * `mediaModels.js` backfills them at load — but `data/media-models.json` is the
 ...
 * running every boot. Same arrangement as migration 237 (disclosures) and 238
 * (Finish edges).
 *
 * Preservation contract (identical to `applyVideoSpeedProfiles`):
 *   - an entry that already has a `speedProfiles` key is left alone (user edit,
 *     including an explicit `null` / `[]` meaning "no profiles on this model")
 *   - an id the user deleted is NOT recreated
 *   - a custom / user-added model is untouched (no shipped profile exists)
 *   - an entry whose `repo` OR `revision` was re-pointed is skipped — a
 *     validated sampler schedule must not be claimed for different weights
 ...
 * a hand-edited bad profile is left for the load-time sanitizer to warn about
...
import { applyVideoSpeedProfiles } from '../../server/lib/videoSpeedProfiles.js';
...
export default {
  async up({ rootDir }) {
    ...
    for (const platform of BUCKET_KEYS) {
      ...
      const next = applyVideoSpeedProfiles(list);
      config.video[platform] = list.map((entry, i) => {
        const gained = Array.isArray(next[i]?.speedProfiles) && !(entry && 'speedProfiles' in entry);
    ...
    if (profiled > 0) {
      ...
      console.log(`📝 ${REL_PATH}: attached speed profiles to ${profiled} video ${profiled === 1 ? 'entry' : 'entries'}`);
    }
  },
};
