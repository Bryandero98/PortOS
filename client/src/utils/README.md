...
| `providers` (agent jobs) | `RUNNABLE_PROVIDER_TYPES` and `filterRunnableProviders` keep the shared CoS agent-job pickers aligned with the server's CLI/TUI file-writing harness boundary while retaining saved pins long enough to clear them. |
