...
- Effort for `agy` is `low`/`medium`/`high`. An effort saved against a Claude or Codex provider clamps down to the nearest level `agy` accepts when you switch a task's provider (`max` and `xhigh` become `high`, `minimal` becomes `low`) instead of being dropped, so switching providers doesn't silently reset the setting.
- The model list comes from `agy models` — click Refresh Models on the AI Providers page and it pulls whatever your installed binary actually offers, rather than a list baked into PortOS that goes stale. Existing installs get the shipped catalog added automatically on first read; a model list you'd customized yourself is left alone.
