...
**Non-walk rows are user data, stored per install, and authored from the UI.** They live in `data/sprites/animation-tracks.json` (`server/services/sprites/animationTrackStore.js` merges them with `walk` into the effective registry), and Sprite Manager's **Animation types** drawer is the CRUD surface over them (`GET/POST/PUT/DELETE /api/sprites/animation-tracks`, `server/services/sprites/animationTrackCrud.js`). `scanner` and `ambient` ship as **seed rows** in `data.reference/sprites/animation-tracks.json` — a user may retune their bounds, reword their prompts, or delete them outright, and may add their own ("a chest opening", "a flower blossoming", "a jetpack burst") without a PortOS code change. Two consequences worth stating plainly:
...
- **A row's `selectionKind` / `setKind` are on-disk contracts, not labels.** The compiler re-verifies an approved set by those exact strings, so renaming them on a track you have already authored orphans that set. This is why the drawer never lets you type them: it derives all five discriminators (`contractFrameCountField`, `contractFpsField`, `selectionKind`, `setKind`, `finalErrorCode`) plus `standaloneContract` from the track id, carries them over verbatim when you edit an existing row, refuses a delete or a facing-mode flip while any record still holds approved work for that track (409 naming the records — there is no force flag), and treats a track's `id` as immutable after creation. Adding, deleting, or retuning a row takes effect immediately for generate/approve, but a **newly-added** track's `runtimeContract` field is registered at server start, so publishing with it needs a restart (the drawer says so).
...
One generic authoring path serves every non-walk track — `POST /api/sprites/:id/tracks/:trackId/generate` and `/approve`, resolved against that track's registry row (`server/services/sprites/animationTrackWorkflow.js`). Walk keeps its own `/walk/*` endpoints because it additionally carries reprocessing, loop trims, per-direction reopen and set-level cycle targets that no other track has.
...
| `server/services/sprites/animationTrackCrud.js` | The WRITE side of that store (#3153): derives the discriminators from the id, refuses a built-in / colliding / in-use mutation, serializes writes and drops the registry cache. Separate from the read-side store so `validation.js`'s import graph stays clear of the record scan the in-use refusal needs |
